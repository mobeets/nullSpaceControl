function [M1, M2, m0, V, mdpt] = current_regression(U, X, Xtarget)
% For position only state: initialize M2, m0 by assuming M1 = I and

% Assume the monkey aims from the current cursor positino straight to the 
% target with a constant speed corresponding to the
% mean-distance-per-timestep (mdpt).
